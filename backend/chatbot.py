...
class InteractiveRAGChatbot:
    ...
    def process_daily_updates(self, updates: List[dict], employee_names: dict = None) -> str:
        ...
        for update in updates:
            ...
            # Handle 'blockers' which can be a list or a string
            blockers_data = update.get('blockers')
            if blockers_data and isinstance(blockers_data, list):
                # Filter out empty strings and format the list into a bulleted string
                blockers_text = "\n".join(f"- {b.strip()}" for b in blockers_data if b and b.strip())
                if not blockers_text:
                    blockers_text = 'No blockers reported'
            elif blockers_data and isinstance(blockers_data, str) and blockers_data.strip():
                blockers_text = f"- {blockers_data.strip()}"
            else:
                blockers_text = 'No blockers reported'
    
            # Handle 'update' (tasks) which can be a list or a string
            tasks_data = update.get('update')
            if tasks_data and isinstance(tasks_data, list):
                # Filter out empty strings and format the list into a bulleted string
                tasks_text = "\n".join(f"- {t.strip()}" for t in tasks_data if t and t.strip())
                if not tasks_text:
                    tasks_text = 'No tasks reported'
            elif tasks_data and isinstance(tasks_data, str) and tasks_data.strip():
                tasks_text = f"- {tasks_data.strip()}"
            else:
                tasks_text = 'No tasks reported'
